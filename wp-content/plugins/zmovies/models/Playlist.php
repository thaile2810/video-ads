...
class Playlist{
	...
	public function channelAddPlaylist($params, $settings = null){
	    
	    if(empty($params['ycnid'])) return false;
	    set_time_limit(3600);
	    $ytube         = new Youtube();
	    $playlists     = $ytube->channel($params['ycnid'],@$params['nextPageToken']);
	    ...
	    if($taxnow == $this->_key) {
	        ...
	        if($action ==  $this->getActionName('update-video') && !empty($terms)){
	            ...
	            if ( 'post' != $post_type )
	                ...
	                if (@$referer && strpos($referer, 'edit-tags.php' )  !== false) {
	                    $location = $referer;
	                }
	...
	public function autoRun(){
	    ...
	    $sql = "SELECT t.term_id,tm2.meta_value AS yurl FROM $wpdb->terms AS t
        	    ...
        	    AND tm2.meta_key = '".$this->metaKey('url')."'
    	        AND tm.meta_value = 2
    	        ORDER BY t.term_id ASC
                LIMIT 1";
	    $terms = $wpdb->get_row($sql, ARRAY_A );
	    
	    $terms['yurl'] = $this->url2Id(@$terms['yurl']);
	    
        if(!empty($terms['yurl'])){
    	    $model = $zController->getModel('CronEvent');
    	    
    	    $filename = ZMOVIES_FILE_PATH . DS . 'logs' . DS . 'CronEvent.log';
