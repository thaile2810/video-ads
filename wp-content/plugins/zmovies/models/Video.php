...
class Video{
	...
	public function playlistAddVideo($params, &$settings = null){
	    ...
	    $filename = ZMOVIES_FILE_PATH . DS . 'logs' . DS . 'items.log';
...
// 	            add_filter('title_save_pre', 'empty_filter');
	...
	public function registerStatus(){
	    ...
	    register_post_status( 'report_die', array(
	        'label'                     => _x( 'Report video die', 'post' ),
	        'public'                    => true,
	        'show_in_admin_all_list'    => true,
	        'show_in_admin_status_list' => true,
	        'label_count'               => _n_noop( 'Report video die <span class="count">(%s)</span>', 'Report video die <span class="count">(%s)</span>' )
	    ) );
	    register_post_status( 'video_die', array(
	        'label'                     => _x( 'Video die', 'post' ),
	        'public'                    => true,
	        'show_in_admin_all_list'    => true,
	        'show_in_admin_status_list' => true,
	        'label_count'               => _n_noop( 'Video die <span class="count">(%s)</span>', 'Video die <span class="count">(%s)</span>' )
	    ) );
	}
	...
	public function displayPostStatus( $states ) {
	    ...
	    if($arg != 'pending'){
	        ...
	        if($post->post_status == 'report_die'){
	            return array('Report video die');
	        }
	        if($post->post_status == 'video_die'){
	            return array('Video die');
	        }
	    }
	    return $states;
	}
	...
	public function appendPostStatus(){
	    ...
	    if($post->post_type == 'post'){
	        ...
	        if($post->post_status == 'report_die'){
	            $completeNew3 = ' selected="selected"';
	            $label3 = '<span id="post-status-display">Report video die</span>';
	            $save3  = '$("#save-post").val("Save report video die"); ';
	        }

	        if($post->post_status == 'video_die'){
	            $completeNew4 = ' selected="selected"';
	            $label4 = '<span id="post-status-display">Video die</span>';
	            $save4  = '$("#save-post").val("Save Video die"); ';
	        }
	        ...
	        echo '
                ...
                jQuery(document).ready(function($){
                    ...
                    ' . $save2 . ' 
                    $("select#post_status").append(\'<option value="report_die" '.$completeNew3.'>Report Video Die</option>\');
                    $(".misc-pub-section label").append(\''.$label3.'\');
                    ' . $save3 . '
                    $("select#post_status").append(\'<option value="video_die" '.$completeNew4.'>Video Die</option>\');
                    $(".misc-pub-section label").append(\''.$label4.'\');
                    ' . $save4 . '
	...
	public function appendQuickEditStatus() {
	    echo "<script>
            jQuery(document).ready( function($) {
                ...
                $( 'select[name=\"_status\"]' ).append( '<option value=\"report_die\">Report Video Die</option>' );
                $( 'select[name=\"_status\"]' ).append( '<option value=\"video_die\">Video Die</option>' );
        	});
        	</script>";
	}
	...
	public function custom_bulk_action() {
		global $typenow, $pagenow, $zController;
			...
			$wp_list_table = _get_list_table('WP_Posts_List_Table');
	  		...
	  		if($action == 'edit'){
	  			foreach ($_REQUEST['post'] as $key => $value) {
	            	...
	            	$model->action_pre_quick_edit_update($value, $_REQUEST['_status']);
	  			}
	  		}
	  	}
	}

	public function action_pre_post_update( $post_id, $data ) {
		$data_post = get_post( $post_id, ARRAY_A );
		// echo '<pre>';
		// print_r($data_post);
		// echo '</pre>';
		...
		// print_r($data);
		// echo '</pre>';
		if($data_post['post_status'] == 'report_16'){
			if($data['post_status'] != 'video_16'){
				global $wpdb;
				$user_report_16 = get_post_meta( $post_id, 'zvideos_video_report_16_by_user', true );
				foreach ($user_report_16 as $key => $value) {
					$user_spam = get_user_meta( $value, 'zvideos_video_spam_report_16', true );
					if($user_spam == ''){
						$user_spam = [$post_id];
					}else{
						$user_spam[] = $post_id;
					}
					update_user_meta( $value, 'zvideos_video_spam_report_16', $user_spam );
				}
			}
		}else if($data_post['post_status'] == 'report_die'){
			if($data['post_status'] == 'video_die'){
				$author_spam = get_user_meta( $data['post_author'], 'zvideos_video_upload_post_die', true );
				if($author_spam == ''){
					$author_spam = [$post_id];
				}else{
					$author_spam[] = $post_id;
				...
				update_user_meta( $data['post_author'], 'zvideos_video_upload_post_die', $author_spam );
			}else{
				global $wpdb;
				$user_report_die = get_post_meta( $post_id, 'zvideos_video_report_die_by_user', true );
				foreach ($user_report_die as $key => $value) {
					$user_spam = get_user_meta( $value, 'zvideos_video_spam_report_die', true );
					if($user_spam == ''){
						$user_spam = [$post_id];
					}else{
						$user_spam[] = $post_id;
					}
					update_user_meta( $value, 'zvideos_video_spam_report_die', $user_spam );
				}
			}
		}
	}

	public function action_pre_quick_edit_update( $post_id, $post_status ) {
		$data_post = get_post( $post_id, ARRAY_A );
		if($data_post['post_status'] == 'report_16'){
			if($post_status != 'video_16'){
				global $wpdb;
				$user_report_16 = get_post_meta( $post_id, 'zvideos_video_report_16_by_user', true );
				foreach ($user_report_16 as $key => $value) {
					$user_spam = get_user_meta( $value, 'zvideos_video_spam_report_16', true );
