...
class Channel{
	...
	public function userAddChannel($params, $setting = null){
	    global $wpdb,$zController;
	    
	    if(!isset($params['yuid'])) return false;
	    
	    $ytube         = new Youtube();
	    $channels      = $ytube->user($params['yuid']);
	            ...
	            if($channel['kind'] == 'youtube#channel'){
	                ...
	                if(empty($term)){
	                    ...
	                    if($term && !is_wp_error($term)){
	                        ...
	                        update_term_meta($term['term_id'],$this->metaKey('ycnid'), $channel_yid);
...
// 	            $model             = $zController->getModel('Playlist');
	...
	public function autoRun(){
	    ...
	    $sql = "SELECT t.term_id,tm2.meta_value AS ycnid FROM $wpdb->terms AS t
        	    ...
        	    AND tm2.meta_key = '".$this->metaKey('ycnid')."'
        	    AND tm.meta_value = 2
