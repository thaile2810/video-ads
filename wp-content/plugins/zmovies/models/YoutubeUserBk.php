...
//             		}
	...
	public function daily($autoStd,$options = null){
	    ...
	    /* $query = "UPDATE $wpdb->terms
            	    SET meta_value = 2
            	    WHERE meta_key = '" . $this->metaKey('status') . "'
                    AND meta_value = 1"; */
	    ...
	    //if($update){// nếu có data mới add event
	        ...
	        [next_run] => now
	        [schedule] => _oneoff
	        [new_cron] => Add Cron Event */
	        
	        $model->add_cron('now', '_oneoff', $this->_key, [914,512] );
	        //add_action($hook, [$autoStd, '']);
	    //}
	}
	...
	public function autoRun(){
	    ...
	    $sql = "SELECT t.term_id,tm2.meta_value AS ycnid FROM $wpdb->terms AS t
                ...
                LIMIT 1";
        // $filename = ZMOVIES_FILE_PATH . DS . 'logs' . DS . 'zvideos_channel.log';
